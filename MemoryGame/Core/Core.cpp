...
AGK_App*	_g_agk_app					= NULL;
int			_g_agk_device_width			= 640;
int			_g_agk_device_height		= 480;
bool		_g_agk_device_fullscreen	= false;
char*		_g_agk_title				= NULL;
	...
	int       nCmdShow)
{
	...
	int dwCenterWindowX = (GetSystemMetrics(SM_CXSCREEN) - (int)_g_agk_device_width) / 2;
	int dwCenterWindowY = (GetSystemMetrics(SM_CYSCREEN) - (int)_g_agk_device_height) / 2;
	//HWND hWnd = CreateWin32Window(hInstance, _g_agk_device_width, _g_agk_device_height, uString("AGK"), dwCenterWindowX, dwCenterWindowY, _g_agk_device_fullscreen);
	HWND hWnd = CreateWin32Window(hInstance, _g_agk_device_width, _g_agk_device_height, uString(_g_agk_title), dwCenterWindowX, dwCenterWindowY, _g_agk_device_fullscreen);
	...
	while (!bExitLoop)
	{
		...
		else
		{
			...
			try
			{
				if (!agk::IsCapturingImage()) _g_agk_app->Loop();
			}
			catch (...)
			{
				uString err = agk::GetLastError();
